...
const getListGroupByParent = catchAsyncError(async (req, res, next) => {
  ...
  const groupsList = await questionModel.aggregate([
    {
      $group: {
  ...
  const data = await questionModel
    .find({}, "name device_id parent_name")
    .lean();
...
const createData = catchAsyncError(async (req, res, next) => {
  ...
  const lastDoc = await questionModel.find().sort({ _id: -1 });
  if (lastDoc.length > 0) {
...
const updateData = catchAsyncError(async (req, res, next) => {
  ...
  // const childrenParentUpdate = await questionModel.updateMany(
...
async function getAllLeafNodes(data) {
  ...
  let parents = await questionModel.find({
    name: { $ne: "Primary" },
...
//   const leafNodes2 = await questionModel.aggregate([
...
const getLeafDeviceList = catchAsyncError(async (req, res, next) => {
  ...
  const leafNodes = await questionModel.aggregate([
    {
      $lookup: {
