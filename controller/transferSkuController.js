const transferSkuModel = require("../db/models/transferSkuModel");
...
const getDataWithPagination = catchAsyncError(async (req, res, next) => {
    ...
    let totalData = await transferSkuModel.countDocuments(query);
    console.log("totalData=================================", totalData);
    // const data = await transferSkuModel.find(query).skip(startIndex).limit(limit);

    const data = await transferSkuModel.aggregate([
        {
            $match: query,
        },
        {
...
const createData = catchAsyncError(async (req, res, next) => {
    ...
    const lastDoc = await transferSkuModel.find().sort({ _id: -1 });
    let decodedData = jwt.verify(token, process.env.JWT_SECRET);
    let newData = {
        ...req.body,
        created_by: decodedData?.user?.email,
    };
    const data = await transferSkuModel.create(newData);
