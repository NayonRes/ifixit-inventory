...
const getAllStock = catchAsyncError(async (req, res, next) => {
  ...
  const data = await repairAttachedSparepartsModel.find(query);
...
const getDataWithPagination = catchAsyncError(async (req, res, next) => {
  ...
  let totalData = await repairAttachedSparepartsModel.countDocuments(query);
  console.log("totalData=================================", totalData);
  //const data = await repairAttachedSparepartsModel.find(query).skip(startIndex).limit(limit);
...
const updateData = catchAsyncError(async (req, res, next) => {
  ...
  data = await repairAttachedSparepartsModel.findByIdAndUpdate(
    req.params.id,
    newData,
    {
      new: true,
...
const purchaseReturn = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    const matchedRecords = await repairAttachedSparepartsModel
      .find({
        sku_number: { $in: sku_numbers },
      })
      .session(session);
    ...
    if (listOfUpdateStock.length > 0) {
      await repairAttachedSparepartsModel.bulkWrite(listOfUpdateStock, {
        session,
      });
    }
    let newAbortTransaction = false; // Flag to track if transaction should be aborted
